...
def progress_bar(task="Processing"):
    ...
    print(f"\n{Fore.GREEN}{BOLD}✓ Task Completed!{RESET}\n")
...
def get_expire_text():
    ...
    for unit, value in EXPIRE_TIME.items():
        if value > 0:
            name = unit if value > 1 else unit[:-1]
            parts.append(f"{value} {name}")
    return ", ".join(parts) if parts else "Permanently"

def expire_delta_from_file(expire_text):
    try:
        if expire_text == "Permanently":
...
if args == ["--create"]:
    if os.path.exists(FILE_NAME):
        WARN("UID IS ALREADY REGISTERED ON THIS DEVICE!")
        sys.exit(0)

    print(f"\n{Fore.CYAN}{BOLD} SELECT FROM SECONDS-YEARS:{RESET}")
    print(f" {Fore.WHITE}1. Seconds   2. Minutes   3. Hours")
    print(f" {Fore.WHITE}4. Days      5. Months    6. Years")

    try:
        choice = input(f"\n{Fore.WHITE} Enter choice number (1-6): {Fore.YELLOW}")
        ...
        if choice in unit_map:
            unit_name = unit_map[choice]
            amount = int(input(f"{Fore.WHITE} Enter amount of {Fore.CYAN}{unit_name}{Fore.WHITE}: {Fore.YELLOW}"))
            EXPIRE_TIME[unit_name] = amount
            print(f"{RESET}")
        else:
            print("")
    ...
    expire_text = get_expire_text()

    with open(FILE_NAME, "w") as f:
        f.write(f"# YOUR UID INFORMATION\n\n"
                f"UID        : {uid}\n"
                f"PASSWORD   : {xor_encrypt(password, XOR_KEY)} (Password is encrypted!)\n"
                f"DEVICE UID : {device_uid()}\n"
                f"ARCH       : {arch_info()}\n"
                f"CREATED AT : {created.isoformat()}\n"
                f"EXPIRED IN : {expire_text}\n\n"
                f"# Information!\n"
                f"DEVICE UID is a combination of System(OS), Version of the phone, Machine type, Processor, and Hostname, so DEVICE UID cannot be copied carelessly.")
    ...
    INFO(f"EXPIRE IN  : {Fore.WHITE}{expire_text}")
    print(f"{Fore.WHITE}{'─'*50}")
    sys.exit(0)
