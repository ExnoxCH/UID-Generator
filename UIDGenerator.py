...
if args == ["--create"]:
    ...
    print(f"\n{Fore.CYAN}{BOLD} PILIH MULAI DARI DETIK-TAHUN:{RESET}")
    ...
    try:
        ...
        if Input in unit_map:
            unit_name = unit_map[Input]
            jumlah = int(input(f"{Fore.WHITE} Masukkan jumlah {Fore.CYAN}{unit_name}{Fore.WHITE}: {Fore.YELLOW}"))
            EXPIRE_TIME[unit_name] = jumlah
            print(f"{RESET}")
        else:
            print("")
    ...
    with open(FILE_NAME, "w") as f:
        f.write(f"# INFORMASI UID ANDA\n\n"
                ...
                f"# Informasi!\n"
                f"DEVICE UID gabungan dari System(OS), Versi dari hp, Tipe mesin, Posesor, dan Hostname, jadi DEVICE UID tidak bisa dicopy sembarangan")
